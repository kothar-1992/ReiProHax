    ...
    if (!g_Token.empty()) {
        ...
        if (!g_Auth.empty()) {
            ...
            if (g_Token == g_Auth) {
                ...
                esp.DrawTextName(Color::White(255), "Rei Pro - Check FPS : ", Vec2(screenWidth / 12, screenHeight / 13.5), screenHeight / 40);
                ...
                if (response.Success) {
                    ...
                    esp.DrawFillCircle(Color::Black(35), request.radarPos, request.radarSize + (request.radarSize / 10), 0.5);
                    esp.DrawCircle(Color::White(255), request.radarPos, request.radarSize + (request.radarSize / 10), 1);
                        ...
                        if (response.Players[i].HeadLocation.z != 1) {
                            ...
                            if (is360Alert && isOutsideSafeZone(location, screen)) {
                                Vec2 hintDotRenderPos = pushToScreenBorder(location, screen,
                                                                           (mScaleY * 100) / 3,
                                                                           5.0f); // Menggunakan skala 2.0 untuk jarak lebih jauh
                            ...
                            if (boxData.Location.z != 1.0f) {
                                ...
                                for (int ij = 0; ij < boxData.itemCount; ij++) {
                                    if (GetBox((int) boxData.itemID[ij], &itemname)) {
                                        BoxCount++;
                                        esp.DrawDeadBoxItems(Color(), itemname, Vec2(boxData.Location.x, boxData.Location.y - (float) BoxCount * (screenHeight / 55)), textsize);
                                    }
                                }
                            }
                        }
                ...
                if (response.InLobby){
                    sprintf(extra, "LOBBY");
                    int ALERT = 4;
                    esp.DrawOTH2(Vec2(screenWidth / 2 - screenHeight / 5.6, screenHeight / 15.5), ALERT);
                    esp.DrawTexture(Color::White(255), extra, Vec2(screenWidth / 2 + screenHeight / 245, screenHeight / 8.0), screenHeight / 30);
                }else{
                    int ENEM_ICON = 2;
                    int BOT_ICON = 3;
                    ...
                    esp.DrawOTH2(Vec2(screenWidth / 2 - (80), 60), ENEM_ICON);
                    esp.DrawOTH2(Vec2(screenWidth / 2, 60), BOT_ICON);
                    esp.DrawTexture(Color(255, 255, 255, 255), cn,
                                    Vec2(screenWidth / 2 - (35), 100), 23);
                    esp.DrawTexture(Color(255, 255, 255, 255), bt,
                                    Vec2(screenWidth / 2 + (75), 100), 23);
                }
